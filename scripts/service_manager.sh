  ...
  case "$app_name" in
    ...
    desktop)
      ...
      if [ -n "$DISPLAY" ]; then
        ...
        # Use startx without specifying a user - will run as root when called with sudo
        startx &
      fi
      ;;
    *)
...
if [ "$ACTION" = "kill" ]; then
  ...
  case "$SERVICE" in
    ...
    desktop)
      ...
      terminate_app "lxpanel" "true"
      terminate_app "pcmanfm" "true"
      terminate_app "openbox" "true"
      terminate_app "Xorg" "true"
      ;;
    *)
